...
class LiveEngine:
    ...
    def _check_drift(self, slot: StrategySlot):
        ...
        try:
            ...
            if hasattr(result, "drift_detected") and result.drift_detected:
                self._log(f"⚠️  DRIFT detected for {slot.strategy_id}")
        except Exception as e:
    ...
    def _check_kill_switch(self, slot: StrategySlot, tick: BrokerTick):
        ...
        if result.triggered:
            self._log(f"🚨 KILL SWITCH [{slot.strategy_id}]: {result}")
    ...
    def _handle_kill_action(self, slot: StrategySlot, result: Any):
        ...
        if action == KillAction.LIQUIDATE:
            self._log(f"🔴 LIQUIDATING {slot.strategy_id}")
            if slot.mode == "live":
            ...
            self._log(f"📉 REDUCING {slot.strategy_id}")
    ...
    def _check_promotions(self):
        ...
        for sid, slot in self._slots.items():
            ...
            if comparison.get("meets_promotion_criteria", False):
                self._log(f"🎉 {sid} meets promotion criteria!")
                if self._lifecycle:
                    try:
    ...
    def _emergency_shutdown(self):
        ...
        self._log("🔴 EMERGENCY SHUTDOWN — flattening all positions")
        try:
