...
class Phase1Pipeline:
    ...
    def run(
        ...
        for s in strategies:
            ...
            registered += 1
        print(f"  [OK] Registered {registered}/{len(strategies)} strategies")
        ...
        if returns_dict:
            ...
            if len(shared_ids) >= 4:
                ...
                print(f"  PBO = {pbo_result.probability:.2%} "
                      f"({'[OK] OK' if not pbo_result.is_overfit else '[WARN]  OVERFIT'})")
        else:
            print(f"\n[SEARCH] Step 2: Running overfitting analysis...")
            print(f"  (No returns provided -- skipping PBO)")

        # ---- Step 3: Filtering pipeline ----
        print(f"\n[TEST] Step 3: Running filtering pipeline...")
        ...
        print(f"\n[TARGET] Step 4: Running diversification filter...")
        survivor_dicts = []
