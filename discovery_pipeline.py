...
class DiscoveryPipeline:
    ...
    def preflight_check(self) -> bool:
        ...
        if not searxng_ok:
            print("  [WARN] SearXNG not available -- scraping will fail")
        ...
        if not llm_status.get("summarizer"):
            print("  [WARN] Summarizer LLM not available -- extraction will fail")
            print(f"    Fix: Start your model server and ensure '{cfg.llm.summarizer.model}' is loaded")
            all_ok = False
        if not llm_status.get("code_generator"):
            print("  [WARN] Code generator LLM not available -- code gen will fail")
        ...
        try:
            ...
            print(f"[OK] Database OK ({stats['total_documents']} docs, "
        ...
        # 4. Dedup index
        print(f"[OK] Dedup index: {self.dedup.get_index_size()} strategies "
              f"({'FAISS' if self.dedup.use_faiss else 'TF-IDF fallback'})")

        # 5. Output directory
        STRATEGIES_DIR.mkdir(parents=True, exist_ok=True)
        print(f"[OK] Output dir: {STRATEGIES_DIR}")
    ...
    def step_5_validate(self, strategies: List[Dict]) -> List[Dict]:
        ...
        for strat in strategies:
            ...
            if not code:
                logger.info(f"  [FAIL] {name}: No code to validate")
            ...
            if ok:
                ...
                logger.info(f"  [OK] {name}: OK ({trade_count} trades)")
            else:
                ...
                logger.info(f"  [FAIL] {name}: FAILED -- {error_msg}")
    ...
    def _print_summary(self):
        ...
        if s['final_strategies'] > 0:
            print(f"\n[OK] {s['final_strategies']} new strategies ready for backtesting!")
            print(f"  Location: {STRATEGIES_DIR}")
        else:
            print("\n[WARN] No new strategies this run.")
...
Examples:
    ...
    if not args.extract_only:
        ...
        if not ok:
            print("\n[WARN] Preflight failed. Fix issues above or use --extract-only.")
